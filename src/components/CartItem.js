...
const CartItem = ({ item, onDecrease, onIncrease, onDelete }) => {
  ...
  return (
    <tr className="py-[100px] border border-b">
      <td>
        <Image src={item.image} width={100} height={100} className="md:h-[100px] md:w-[100px] h-[50px] w-[50px] m-5 p-5 border cursor-pointer" />
      </td>
      <td className="py-2 px-4 md:w-1/3 w-auto">
        <p className="md:text-sm text-xs text-start w-full text-gray-500">{item.name}</p>
      </td>
      <td className="py-3 lg:px-5 px-2 mx-5 flex flex-row justify-evenly align-center md:mt-[20%] mt-[12%] border border-gray-100">
        ...
        <span className="mx-2 px-2 text-center lg:text-md text-sm">{item.quantity}</span>
